...
function Doctor() {
  ...
  const handleCheckboxChange = (docId, currentStatus) => {
    ...
    AdminAPIwithAcess
      .patch(BASE_URL + `auth/admin/doc/${docId}`, config, formData)
      .then((res) => {
  ...
  const fetchUsers = (url) => {
    ...
    console.log("this portion for the access token", accessToken);
    AdminAPIwithAcess
      .get(url, config)
      .then((req) => {

        const filteredData = req.data.results.filter(
  ...
  const approvedDoctors = doctorData.filter(
  (doctor) => doctor.approval_status === "APPROVED"
);
