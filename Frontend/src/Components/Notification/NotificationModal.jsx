    ...
    useEffect(() => {
      ...
      const fetchNotifications = async () => {
        try {
          ...
          let response = ''
          if(isNaN(customID) && customID.charAt(0) === 'D')
            response = await UserAPIwithAcess.get(`notification/notifications/doctor/${customID}/`, config);
          else if(!isNaN(customID) && !isNaN(parseFloat(customID)))
            response = await UserAPIwithAcess.get(`notification/notifications/patient/${customID}/`, config);
        else
        console.log('wait');
