...
function Notification() {
  ...
  const fetchData = async (customId) => {
    try {
      const response = await axios.get(
        `${BASE_URL}notification/doctor-side/doctor-notification/${customId}/`
      );
  ...
  useEffect(() => {
    ...
    if (!authentication_user.user_id) {
      ...
      axios
        .get(BASE_URL + `auth/custom-id/doctor/${userId}`)
        .then((res) => {
  ...
  const handleNotificationClick = async (notification) => {
    try {
      await axios.put(
        `${BASE_URL}notification/update-notification/${notification.id}/`,
        {
          is_seen: true,
        }
      );
