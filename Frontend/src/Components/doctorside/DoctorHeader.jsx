import React from 'react';
import Logo from '../../assets/logo.png';
import { NavLink, useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { set_Authentication } from '../../Redux/authentication/authenticationSlice';
import Cookies from 'js-cookie';

const DoctorHeader = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();

    const authenticationUser = useSelector((state) => state.authentication_user);
    const { isAuthenticated, user_id } = authenticationUser || {};

    const handleLogOut = () => {
        Cookies.remove('access');
        Cookies.remove('refresh');

        dispatch(
            set_Authentication({
                name: null,
                isAuthenticated: false,
            })
        );
        navigate('/auth/doctor/login');
    };

    return (
        <nav className="flex justify-between items-center py-4 bg-white/80 backdrop-blur-md shadow-md w-full px-10">
            {/* Logo Container */}
            <div className="flex items-center">
                {/* Logo */}
                <a className="cursor-pointer">
                    <img className="h-10 w-auto" src={Logo} alt="Store Logo" />
                </a>
            </div>

            {/* Search bar */}
            <div className="flex items-center w-1/3 relative">
                <input
                    type="text"
                    placeholder="search..."
                    className="w-full px-4 py-2 pr-10 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button className="absolute right-3 top-1/2 transform -translate-y-1/2">
                    <svg className="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-4.35-4.35m1.8-6.15a7.5 7.5 0 11-15 0 7.5 7.5 0 0115 0z"></path>
                    </svg>
                </button>
            </div>

            {/* Navigation Links */}
            <div className="flex space-x-8 font-bold">
                <NavLink to="/doctor/doctorHome" className="text-black hover:text-blue-500 transition-colors duration-300">Home</NavLink>
                <NavLink to="/about-us" className="text-black hover:text-blue-500 transition-colors duration-300">About Us</NavLink>
                <NavLink to="/contact-us" className="text-black hover:text-blue-500 transition-colors duration-300">Contact Us</NavLink>
                {isAuthenticated && (
                    <>
                        <NavLink to="/doctor/profile" className="text-black hover:text-blue-500 transition-colors duration-300">Profile</NavLink>
                        <button onClick={handleLogOut} className="text-black hover:text-blue-500 transition-colors duration-300">Logout</button>
                    </>
                )}
            </div>
        </nav>
    );
};

export default DoctorHeader;
